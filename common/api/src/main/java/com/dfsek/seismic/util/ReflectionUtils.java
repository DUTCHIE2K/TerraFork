...
public class ReflectionUtils {
    ...
    private static @Nullable Field getReflectedField(ClassField clssfild) {
        try {
            ...
            for(Class<?> current = clssfild.clss; current != null; current = current.getSuperclass()) {
                try {
                    return current.getDeclaredField(clssfild.fild());
                } catch(NoSuchFieldException ignoredToo) {
                    // Continue walking the type hierarchy.
                }
            }
